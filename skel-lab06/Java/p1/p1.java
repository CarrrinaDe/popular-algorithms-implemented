...
class Nim {
    ...
    public boolean apply_move(Move move) {
         ...
         */
    	if ((move.heap >= 0) && (move.heap < 3) && (heaps[move.heap] >= move.amount)){
...
class P1 {
    ...
    public static Pair<Integer, Move> minimax(Nim init, int player, int depth) {
         ...
         */
        Nim tmp;
        Pair<Integer, Move> best = new Pair<Integer, Move>(-Nim.Inf, new Move());
        for (Move move : moves) {
        	tmp = (Nim) init.clone();
        	tmp.apply_move(move);
        	Pair<Integer, Move> p = minimax(tmp, -player, depth - 1);
